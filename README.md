### Motivation
This is my work on the final project (Capstone) for the Full Stack Nanodegree of Udacity. It utilizes everything learned during the program, from the python language to the deployment on heroku, through auth0 authentification and relational databases.

### Postman
Included in the project folder is a postman collection of endpoint tests with the bearer token for the Executive Producer authorization.
...
3. Python installation should come with pip. Use "pip --version" to check if it is indeed installed.
4.a If pip is not installed, download it from https://pypi.org/project/pip/#files
...
9. Run "flask run" to deploy the server.
To run the tests, run "python -m unittest test_app.py"
...
- Required Authorization: Casting Director or Executive Producer.
- Request Arguments: <varchar:title>, <datetime:date>, <List<Actor>:actors>; date must be of format: YYYY-MM-DDTHH:MM:SS; actors must be a list of actors.
